...
module_dns() {
    print_to_console "\n>>> [ DNS MODULE ]"
    run_tool "dig_ips" "dig +short ${TARGET_DOMAIN} | grep -oE '([0-9]{1,3}\\.){3}[0-9]{1,3}' | sort -u"
    run_tool "dig_ns" "dig +short NS ${TARGET_DOMAIN}"
    run_tool "dig_mx" "dig +short MX ${TARGET_DOMAIN}"
}
...
module_hping() {
    print_to_console "\n>>> [ TRACEROUTE MODULE ]"
    for IP in $(cat "${LOG_DIR}/dig_ips.txt" 2>/dev/null); do
        run_tool "hping_${IP}" "sudo hping3 --traceroute $DNS_NAME -p $TRACEROUTE_PORT -S -c $TRACEROUTE_MAX_PKTS"
    done
}
...
module_subdomain() {
    ...
    run_tool "subdomain_crtsh" "curl -k -s 'https://crt.sh/?q=%25.${TARGET_DOMAIN}&output=json' | jq -r '.[].name_value' 2>/dev/null | sort -u"
    
    # Verify common subdomains
    COMMON_SUBS="www mail ftp admin api test dev staging"
    for sub in $COMMON_SUBS; do
        host "${sub}.${TARGET_DOMAIN}" 2>/dev/null | grep "has address" | tee -a "${LOG_DIR}/subdomains.txt"
    done
...
module_tech_detection() {
    ...
    # WhatWeb
    run_tool "whatweb" "whatweb -v -color=never --no-errors https://${TARGET_DOMAIN}"
    
    # Wappalyzer local
    run_tool "wappalyzer" "wappalyzer https://${TARGET_DOMAIN} 2>/dev/null | jq . 2>/dev/null || echo 'JSON format not available'"
    
    # Header analysis
    print_to_console "\n[i] Detected Technologies in Headers:"
    curl -k -I "https://${TARGET_DOMAIN}" 2>/dev/null | grep -iE "(server|x-powered-by|asp.net|php|wordpress|drupal|joomla)" | tee "${LOG_DIR}/tech_headers.txt"

    # Waf detection with wafw00f
    run_tool "wafw00f" "$PROXY_CHAINS $WAFW00F https://${TARGET_DOMAIN}"

    # Fetch URLs
    run_tool "spider" "https_proxy=$PROXY spider -v -u \"$USER_AGENT\" -s \
            --domain  https://${TARGET_DOMAIN} scrape 2>/dev/null \
            | grep url | cut -d\" -f4)"
}
...
module_osint() {
    ...
    HARVESTER_CMD="theHarvester -d $TARGET_DOMAIN -n -c -r -f $HARVESTER_FILE -b $HARVESTER_SOURCES"

    run_tool "theHarvester" "$HARVESTER_CMD"

    # nuclei
    run_tool "nuclei" "$NUCLEI_BIN -t http,ssl,misconfiguration,vulnerabilities,cves,file -u $TARGET_DOMAIN -o $NUCLEI_FILE"
}