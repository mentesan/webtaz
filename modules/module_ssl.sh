...
module_ssl() {
    ...
    # Verifica se sslyze existe
    if command -v sslyze &>/dev/null; then
        run_tool "sslyze" "sslyze $TARGET_DOMAIN | sed '/^$/d'"
    else
    ...
    if command -v sslscan &>/dev/null; then
        run_tool "sslscan" "sslscan --no-colour $TARGET_DOMAIN"
        ...
        run_tool "curl_ssl" "curl -k -v --max-time 10 'https://$TARGET_DOMAIN' 2>&1 | grep -i ssl"
    fi
}
